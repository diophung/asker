...
func TestRunServesAndShutsDownGracefully(t *testing.T) {
	...
	go func() {
		done <- run(ctx, appConfig{Addr: "127.0.0.1:0", Subject: "alice@example.com", CodeTTL: time.Minute}, logger, ready)
	}()
...
func TestConfigDefaultsAndEnv(t *testing.T) {
	t.Setenv("FAKE_OAUTH_ADDR", "")
	t.Setenv("FAKE_OAUTH_SUBJECT", "")
	t.Setenv("FAKE_OAUTH_CODE_TTL", "")
	var cfg appConfig
	if err := loadInto(&cfg); err != nil {
		t.Fatal(err)
	}
	if cfg.Addr != ":9500" {
		t.Fatalf("default addr = %q, want :9500", cfg.Addr)
	}
	if cfg.Subject != "alice@example.com" {
		t.Fatalf("default subject = %q, want alice@example.com", cfg.Subject)
	}
	if cfg.CodeTTL != 5*time.Minute {
		t.Fatalf("default code TTL = %v, want 5m", cfg.CodeTTL)
	}

	t.Setenv("FAKE_OAUTH_ADDR", "127.0.0.1:7788")
	t.Setenv("FAKE_OAUTH_SUBJECT", "bob@example.com")
	t.Setenv("FAKE_OAUTH_CODE_TTL", "30s")
	var cfg2 appConfig
	if err := loadInto(&cfg2); err != nil {
		t.Fatal(err)
	}
	if cfg2.Addr != "127.0.0.1:7788" || cfg2.Subject != "bob@example.com" || cfg2.CodeTTL != 30*time.Second {
		t.Fatalf("env overrides not applied: %+v", cfg2)
	}
}

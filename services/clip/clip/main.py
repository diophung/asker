...
log = logging.getLogger("clip.main")
...
def run(config: Config) -> None:
    encoder = OpenClipEncoder(config.model, config.pretrained, config.clip_dim)
    ...
    threading.Thread(target=warmup, name="clip-warmup", daemon=True).start()

    server = make_server(config, encoder, ready=lambda: encoder.loaded)
    log.info(
        "clip service listening",
        extra={
            "host": config.host,
            "port": config.port,
            "model": config.model,
    ...
    def shutdown(_signum: int, _frame: object) -> None:
        log.info("clip service stopping")
        threading.Thread(target=server.shutdown, name="clip-shutdown", daemon=True).start()
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="clip", description="Asker CLIP model service (ADR-013)")
    parser.add_argument(
        "-healthcheck",
        "--healthcheck",
        action="store_true",
    ...
    try:
        ...
        log.exception("clip service crashed")
        return 1
    return 0
